...
pub unsafe fn ymm_eq(a: __m256i, b: __m256i) -> bool {
...
/// Shifts the ymms up
#[inline]
pub unsafe fn shift_up_1(ymm: __m256i) -> __m256i {
    _mm256_permutevar8x32_epi32(ymm, _mm256_set_epi32(7, 7, 6, 5, 4, 3, 2, 1))
}
...
/// Shift ymms to the left
#[inline]
pub unsafe fn shift_right_1(ymm: __m256i) -> __m256i {
    _mm256_srli_epi32::<1>(ymm)
}

/// Print ymm
pub fn ymm_str(ymm: __m256i) -> String {
...
mod tests {
    ...
    fn shift_up_test() {
        let ymm1 = unsafe { ymm([0, 1, 2, 3, 4, 5, 6, 7]) };
        let expected = unsafe { ymm([1, 2, 3, 4, 5, 6, 7, 7]) };
        let rotated = unsafe { shift_up_1(ymm1) };
